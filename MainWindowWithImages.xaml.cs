...
// NOTE: this script runs alongside the BodyBasics-WPF code from Microsoft Kinect SDK 2.0
// To use, replace the MainWindow.xaml.cs file with this

// additional references:
// https://www.dotnetperls.com/timer
// https://www.codeproject.com/Questions/1083027/How-to-write-data-in-json-file-through-csharp
//------------------------------------------------------------------------------
...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        */
        ...
        {
            public float[] rAnkle { get; set; }
            public float[] rKnee { get; set; }
            public float[] rHip { get; set; }
            public float[] lHip { get; set; }
            public float[] lKnee { get; set; }
            public float[] lAnkle { get; set; }
            public float[] pelvis { get; set; }
            public float[] midSpine { get; set; }
            public float[] neck { get; set; }
            public float[] head { get; set; }
            public float[] rWrist { get; set; }
            public float[] rElbow { get; set; }
            public float[] rShoulder { get; set; }
            public float[] lShoulder { get; set; }
            public float[] lElbow { get; set; }
            public float[] lWrist { get; set; }
        }
        ...
        private void WriteJointsToJson(IReadOnlyDictionary<JointType, Joint> joints)
        {
            float[] JointList(JointType jointType)
            {
                if (joints != null)
                {
                    return new float[] { joints[jointType].Position.X, joints[jointType].Position.Y, joints[jointType].Position.Z };
                }
                else
                {
            ...
            {
                rAnkle = JointList(JointType.AnkleRight),
                rKnee = JointList(JointType.KneeRight),
                rHip = JointList(JointType.HipRight),
                lHip = JointList(JointType.HipLeft),
                lKnee = JointList(JointType.KneeLeft),
                lAnkle = JointList(JointType.AnkleLeft),
                pelvis = JointList(JointType.SpineBase),
                midSpine = JointList(JointType.SpineMid),
                neck = JointList(JointType.Neck),
                head = JointList(JointType.Head),
                rWrist = JointList(JointType.WristRight),
                rElbow = JointList(JointType.ElbowRight),
                rShoulder = JointList(JointType.ShoulderRight),
                lShoulder = JointList(JointType.ShoulderLeft),
                lElbow = JointList(JointType.ElbowLeft),
                lWrist = JointList(JointType.WristLeft)
            };
        ...
        */
        ...
        private void MainWindow_Closing(object sender, CancelEventArgs e)
        {
            ...
            if (this.kinectSensor != null)
            {
                ...
                string json = JsonConvert.SerializeObject(jsonJointData.ToArray());
        ...
        private void Reader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            if (dataReceived)
            {
                ...
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    string countString = ((count - 1) % 8 + 1).ToString();
